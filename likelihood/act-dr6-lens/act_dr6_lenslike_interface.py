try:
    ...
    raise RuntimeError(
        "The act_dr6_lenslike python module is required for the act_dr6_lenslike likelihood. Try running: pip install act_dr6_lenslike."
    )
...
def setup(options):
    ...
    like_corrections = options.get_bool(
        option_section, "like_corrections", default=True
    )
    like_only = options.get_bool(option_section, "like_only", default=False)

    data_directory = os.path.join(dirname, "data/v1.1/")
    data_directory = options.get_string(
        option_section, "data_directory", default=data_directory
    )

    if not os.path.exists(data_directory):
        raise FileNotFoundError(
            "Required data file not found at {}.\nPlease obtain it and place it correctly.\nThe script get-act-data.sh will download and place it.".format(
                data_file
            )
        )
    ...
    nsims_act = options.get_int(
        option_section, "nsims_act", default=792
    )  # Number of sims used for covmat; used in Hartlap correction
    nsims_planck = options.get_int(
        option_section, "nsims_planck", default=400
    )  # Number of sims used for covmat; used in Hartlap correction
    ...
    varying_cmb_alens = options.get_bool(
        option_section, "varying_cmb_alens", default=False
    )  # Whether to divide the theory spectrum by Alens
    ...
    data_dict = act_dr6_lenslike.load_data(
        ddir=data_directory,
        variant=variant,
        lens_only=lens_only,
        like_corrections=like_corrections,
        apply_hartlap=apply_hartlap,
        mock=mock,
        nsims_act=nsims_act,
        nsims_planck=nsims_planck,
        trim_lmax=trim_lmax,
        scale_cov=scale_cov,
    )
...
def execute(block, config):
    ...
    lnlike, bclkk = act_dr6_lenslike.generic_lnlike(
        data_dict,
        ell,
        cl_kk,
        ell,
